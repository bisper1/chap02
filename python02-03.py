...
# 리스트 안의 리스트의 요소에 접근하려면 자바의 2차원 배열을 사용하는 것처럼 접근이 가능
# 출력 : a[1번째리스트 index][2번째리스트 index]
# 입력 : a[1번째리스트 index][2번째리스트 index] = 값
a = [1, 2, 3, ["a", "b", "c"]]

print("리스트 a의 2번째 요소 출력 : {0}".format(a[2]))
print("리스트 a의 3번째 요소 출력 : {0}".format(a[3]))
print("리스트 a의 3번째 요소의 1번째 요소 출력 : {0}".format(a[3][1]))
...
print()
print("리스트a의 1:2 값 출력 : {0}".format(a[1:2]))
a[1:2] = ["a", "b", "c"]
...
# 리스트 요소 삭제
# 리스트의 요소에 빈 리스트를 입력하며 해당 리스트의 인덱스가 삭제 됨
print()
