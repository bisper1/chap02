...
# 변수
# C/C++나 자바와 같은 다른 언어와 달리 변수 선언 시 자료형을 입력할 필요가 없음
# C#, javascript 에는 var 타입이 존재함
# php 는 변수 선언 시 $로 통일함
# 위의 언어들은 모두 값의 자료형을 자동을 유추함
...
# 변수를 리스트 형태로 선언하고 값도 리스트 행태로 대입함
print()
[a, b] = ["python", "life"]
print("변수 a의 값 : {0}".format(a))
print("변수 b의 값 : {0}".format(b))
...
# print("dle 사용 후의 b의 참조 카운트 : {0}".format(sys.getrefcount(b)))
